        ...
        MPIX_Comm* mpi_comm)
{
    ...
    for (int i = 0; i < PPN; i++)
    {
        ...
        if (local_send_displs[i] <= rank_node && local_send_displs[i+1] > rank_node)
            local_idx = i;
    }
    ...
    for (int i = 0; i < PPN; i++)
    {
        ...
        start = local_send_displs[send_proc];
        end = local_send_displs[send_proc+1];
        MPI_Sendrecv(&(send_buffer[start*send_msg_size*send_size]),
                     (end - start) * send_msg_size,
                     sendtype,
                     send_proc,
                     tag,
                     &(tmpbuf[recv_proc*local_num_msgs*send_msg_size*send_size]),
     ...
     ************************************************/
    // Reorder
    for (int i = 0; i < PPN*local_num_msgs; i++)
    {
        for (int j = 0; j < PPN; j++)
        {
            for (int k = 0; k < send_msg_size*send_size; k++)
            {
                contig_buf[(i*PPN+j)*send_msg_size*send_size + k] = 
                        tmpbuf[(j*PPN*local_num_msgs+i)*send_msg_size*send_size + k];
            }
        }
    }
