                          ...
                          MPIX_Comm comm)
{
    ...
    MPI_Gather(sendbuf, sendsize, sendtype, temp_buffer, recvcount, recvtype, 0, comm.local_comm);

    if (local_rank == 0)
    {
        MPI_Allgather(temp_buffer, procs_per_leader * sendsize, sendtype, recv_buffer, procs_per_leader * recvcount, recvtype, comm.leader_comm);
    }

    MPI_Bcast(recv_buffer, num_procs, recvtype, 0, comm.local_comm);
