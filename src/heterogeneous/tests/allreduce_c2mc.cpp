        ...
        MPI_Comm comm, int n_iters)
{
    ...
    {
        gpuMemcpyAsync(sendbuf, sendbuf_d, size*sizeof(float), gpuMemcpyDeviceToHost, stream);
...
//        gpuMemcpy(recvbuf_d, recvbuf, size*sizeof(float), gpuMemcpyHostToDevice);
        gpuMemcpyAsync(recvbuf_d, recvbuf, size*sizeof(float), gpuMemcpyHostToDevice, stream);
        cudaStreamSynchronize(stream);
    }
        ...
        MPI_Comm comm, int ppg, int socket_rank)
{
    ...
    { 
        ...
        // Compare Results
        // 1. Multi-Proc
        allreduce(s, &(sendbuf_d[s*socket_rank]), sendbuf, 
                &(recvbuf_d[s*socket_rank]), recvbuf, comm, 1);
        ...
        // Warm-Up
        allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, comm, 1);

        // Time 2 iterations
        time = allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, comm, 2);
        ...
        // Time Allreduce
        time = allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, comm, n_iters);
