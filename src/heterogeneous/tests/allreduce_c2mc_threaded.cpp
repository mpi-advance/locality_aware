...
double allreduce(int size, float* sendbuf_d, float* sendbuf, float* recvbuf_d, float* recvbuf, 
        MPI_Op& operation, MPI_Comm comm, int n_iters)
{
        ...
        MPI_Comm comm, int ppg, int socket_rank)
{
    ...
    { 
        ...
        allreduce(s, &(sendbuf_d[s*socket_rank]), sendbuf, 
                &(recvbuf_d[s*socket_rank]), recvbuf, operation, comm, 1);
        ...
        // Warm-Up
        allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, operation, comm, 1);

        // Time 2 iterations
        time = allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, operation, comm, 2);
        ...
        // Time Allreduce
        time = allreduce(s, &(sendbuf_d[socket_rank*s]), sendbuf, &(recvbuf_d[socket_rank*s]), recvbuf, operation, comm, n_iters);
