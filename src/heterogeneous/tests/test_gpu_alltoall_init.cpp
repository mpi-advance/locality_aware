...
TEST(RandomCommTest, TestsInTests)
{
    ...
    std::vector<int> advance_alltoall(max_s*num_procs);
    ...
    for (int i = 0; i < max_i; i++)
    {
        ...
        alltoall_init_nonblocking(local_data.data(),
                s,
                MPI_INT,
                advance_alltoall.data(),
                s,
                MPI_INT,
                MPI_COMM_WORLD,
                MPI_INFO_NULL,
                &xreq);
        MPIX_Start(xreq);
        MPIX_Wait(xreq, &status);
        MPIX_Request_free(xreq);
        for (int j = 0; j < s*num_procs; j++)
            ASSERT_EQ(std_alltoall[j], advance_alltoall[j]);

        // Pairwise Alltoall
        alltoall_init_stride(local_data.data(), 
                s,
                MPI_INT, 
                advance_alltoall.data(), 
                s, 
        ...
        gpu_aware_alltoall_init_nonblocking(local_data_d,
                s,
                MPI_INT,
                alltoall_d,
                s,
        ...
        MPIX_Wait(xreq, &status);
        MPIX_Request_free(xreq);
        gpuMemcpy(advance_alltoall.data(),
                alltoall_d,
                s*num_procs*sizeof(int),
