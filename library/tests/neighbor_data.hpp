...
template <typename U>
void form_initial_communicator(int local_size,
                               MPIL_Data<U>* send_data,
                               MPIL_Data<U>* recv_data)
{
    // Get MPI Information
    int rank, num_procs;
    ...
    MPI_Allreduce(
        MPI_IN_PLACE, comm_procs.data(), num_procs, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
template <typename U>
void form_global_indices(int local_size,
                         MPIL_Data<U> send_data,
                         MPIL_Data<U> recv_data,
                         std::vector<long>& global_send_idx,
                         std::vector<long>& global_recv_idx)
{
    int rank, num_procs;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &num_procs);
    ...
    for (int i = 0; i < send_data.num_msgs; i++)
    {
        ...
        MPI_Isend(&(global_send_idx[start]),
                  (int)(end - start),
                  MPI_LONG,
                  proc,
                  tag,
    ...
    for (int i = 0; i < recv_data.num_msgs; i++)
    {
        ...
        MPI_Irecv(&(global_recv_idx[start]),
                  (int)(end - start),
                  MPI_LONG,
                  proc,
                  tag,
                  MPI_COMM_WORLD,
                  &(recv_data.requests[i]));
    }
    if (send_data.num_msgs)
    {
        MPI_Waitall(send_data.num_msgs, send_data.requests.data(), MPI_STATUSES_IGNORE);
