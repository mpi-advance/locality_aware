...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < max_i; i++)
    {
        ...
        // Persistent Bruck on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_GPU_BRUCK);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
                        s,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent GPU Bruck equivalent to PMPI\n");

        // Persistent Ring on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_GPU_RING);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent GPU Ring equivalent to PMPI\n");


#if defined(MPI4)
        // Persistent PMPI on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_GPU_PMPI);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent GPU PMPI equivalent to PMPI\n");
#endif
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("CTC Bruck equivalent to PMPI\n");

        // Persistent Bruck on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_CTC_BRUCK);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent CTC Bruck equivalent to PMPI\n");

        // Persistent Ring on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_CTC_RING);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent CTC Ring equivalent to PMPI\n");


#if defined(MPI4)
        // Persistent PMPI on GPU
        MPIL_Set_allgather_init_algorithm(ALLGATHER_INIT_CTC_PMPI);
        MPIL_Allgather_init(local_data_d,
                        s,
                        MPI_INT,
                        allgather_d,
        ...
        ierr = gpuStreamSynchronize(0);
        gpu_check(ierr);
        if (rank == 0) printf("Persistent CTC PMPI equivalent to PMPI\n");
#endif
