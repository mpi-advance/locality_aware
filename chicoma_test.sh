...
do
    ...
    echo "#SBATCH --tasks-per-node=128" >> temp_sbatch
    echo "#SBATCH --cpus-per-task=1" >> temp_sbatch
    echo "#SBATCH --time=1:30:00" >> temp_sbatch
    echo "#SBATCH --sockets-per-node=2" >> temp_sbatch
    echo "#SBATCH --cores-per-socket=64" >> temp_sbatch
    echo "#SBATCH --partition=standard" >> temp_sbatch
    ...
    echo "export FI_CXI_RX_MATCH_MODE=software" >> temp_sbatch    
