...
#define PPNUMA 24 // number of processes per NUMA region
#define PPS 24  //number of processes per socket
#define PPN 96 //number of processes per node
...
void ping_managed(char* sendbuf_m, char* sendbuf_h, char* recvbuf_m, char* recvbuf_h, int size, int proc,
        gpuStream_t stream, MPI_Comm comm)
{  
    // Move to CPU
    gpuMemPrefetchAsync(sendbuf_m, size*sizeof(char), hipCpuDeviceId, stream);
    gpuMemPrefetchAsync(recvbuf_m, size*sizeof(char), hipCpuDeviceId, stream);
    gpuStreamSynchronize(stream);

    // Send between CPUs
    MPI_Send(sendbuf_m, size, MPI_CHAR, proc, 0, comm);
    MPI_Recv(recvbuf_m, size, MPI_CHAR, proc, 0, comm, MPI_STATUS_IGNORE);

    // Move to GPU
    gpuMemPrefetchAsync(sendbuf_m, size*sizeof(char), device, stream);
    gpuMemPrefetchAsync(recvbuf_m, size*sizeof(char), device, stream);
    gpuStreamSynchronize(stream);
}

void pong_managed(char* sendbuf_m, char* sendbuf_h, char* recvbuf_m, char* recvbuf_h, int size, int proc,
        gpuStream_t stream, MPI_Comm comm)
{
    // Move to CPU
    gpuMemPrefetchAsync(sendbuf_m, size*sizeof(char), hipCpuDeviceId, stream);
        ...
        gpuStream_t stream, MPI_Comm comm)
{
    ...
    if (GPNUMA <= 1)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-NUMA, because GPNUMA is set to %d\n", GPNUMA);
    }
    else if (GPNUMA == GPN)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-NUMA, because only 1 NUMA per node\n");
    }
    else
    {
        if (rank == 0) printf("On-NUMA MultiProc Ping-Pong:\n");
        size = GPNUMA/2;
        cond0 = rank/PPG < size && rank%PPG==0;
        cond1 = rank/PPG < 2*size && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPNUMA/2, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (GPS <= 1)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket, because GPS is set to %d\n", GPS);
    }
    else if (GPS == GPN)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket, because only 1 socket per node\n");
    }
    else if (GPNUMA == GPS)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket because same as intra-NUMA\n");
    }
    else
    {
        ...
        cond0 = rank/PPG < size && rank%PPG==0;
        cond1 = rank/PPG < 2*size && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPS/2, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (GPN <= 1)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-node, because GPN is set to %d\n", GPN);
    }
    else
    {
        ...
        cond0 = rank/PPG < size && rank%PPG==0;
        cond1 = rank/PPG < 2*size && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPN/2, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (NODES < 2)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc internode, because NODES is set to %d\n", NODES);
    }
    else
    {
        ...
        cond0 = rank/PPG < size && rank%PPG==0;
        cond1 = rank/PPG < 2*size && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPN, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (GPNUMA <= 1)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-NUMA with all numas active , because GPNUMA is set to %d\n", GPNUMA);
    }
    else if (GPNUMA == GPN)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-NUMA with all numas active, because only 1 NUMA per node\n");
    }
    else
    {
        if (rank == 0) printf("On-NUMA MultiProc Ping-Pong, All NUMAs Active:\n");
        size = GPNUMA/2;
        cond0 = rank/PPG < GPN && ((rank/PPG) % GPNUMA) < size && rank%PPG==0;
        cond1 = rank/PPG < GPN && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPNUMA/2, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (GPS <= 1)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket with all sockets active, because GPS is set to %d\n", GPS);
    }
    else if (GPS == GPN)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket with all sockets active, because only 1 socket per node\n");
    }
    else if (GPNUMA == GPS)
    {
        if (rank == 0) printf("Not measuring GPU-Aware multiproc intra-socket with all sockets active, because same as intra-NUMA\n");
    }
    else
    {
        if  (rank == 0) printf("On-Socket MultiProc Ping-Pong, All Sockets Active:\n");
        size = GPS/2;
        cond0 = rank/PPG < GPN && ((rank/PPG) % GPS) < size && rank%PPG==0;
        cond1 = rank/PPG < GPN && rank%PPG==0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPS/2, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (NODES < 2)
    {
        if (rank == 0) printf("Not measuring GPU-Aware off-node multiproc with even numa regions, because NODES is set to %d\n", NODES);
    }
    else if (GPN == GPNUMA)
    {
        if (rank == 0) printf("Not measuring GPU-Aware off-node multiproc with even numa regions, because only one NUMA per node\n");
    }
    else
    {
        if (rank == 0) printf("Off-Node MultiProc Ping-Pong, Even NUMA Regions:\n");
        size = GPNUMA;
        cond0 = rank/PPG < GPN && rank%PPG==0;
        cond1 = rank/PPG < 2*GPN && rank%PPG == 0;
        multiproc_ping_pong_step(f_ping, f_pong, max_p, sendbuf_d, sendbuf_h, recvbuf_d, recvbuf_h, size, PPN, PPG, cond0, cond1,
                stream, comm);
    }
    ...
    if (NODES < 2)
    {
        if (rank == 0) printf("Not measuring GPU-Aware off-node multiproc with even sockets, because NODES is set to %d\n", NODES);
    }
    else if (GPN == GPS)
    {
        if (rank == 0) printf("Not measuring GPU-Aware off-node multiproc with even sockets, because only one socket per node\n");
    }
    else if (GPS == GPNUMA)
    {
        if (rank == 0) printf("Not measuring GPU-Aware off-node multiproc with even sockets, because it is the same as even NUMA regions\n");
    }
    else
    {
...
int main(int argc, char* argv[])
{
    ...
    gpuMallocManaged((void**)&sendbuf_m, max_s*sizeof(char));
    gpuMallocManaged((void**)&recvbuf_m, max_s*sizeof(char));
    ...
    // Make sure data is already on GPU before calling this
    if (rank == 0) printf("Running standard GPU-Aware benchmarks on managed memory\n");
    gpuMemPrefetchAsync(sendbuf_m, max_s*sizeof(char), device, stream);
    gpuMemPrefetchAsync(recvbuf_m, max_s*sizeof(char), device, stream);
    gpuStreamSynchronize(stream);
    standard_ping_pong_gpu(ping_gpu_aware, pong_gpu_aware, max_p, sendbuf_m, NULL,
            recvbuf_m, NULL, stream, MPI_COMM_WORLD);

    // Make sure data is already on CPU before calling this
    if (rank == 0) printf("Running standard inter-CPU benchmarks on managed memory\n");
    gpuMemPrefetchAsync(sendbuf_m, max_s*sizeof(char), hipCpuDeviceId, stream);
    gpuMemPrefetchAsync(recvbuf_m, max_s*sizeof(char), hipCpuDeviceId, stream);
    gpuStreamSynchronize(stream);
    standard_ping_pong_gpu(ping_gpu_aware, pong_gpu_aware, max_p, sendbuf_m, NULL,
            recvbuf_m, NULL, stream, MPI_COMM_WORLD);
    ...
    if (rank == 0) printf("Running standard GPU Managed benchmarks\n");
    gpuMemPrefetchAsync(sendbuf_m, max_s*sizeof(char), device, stream);
    gpuMemPrefetchAsync(recvbuf_m, max_s*sizeof(char), device, stream);
    gpuStreamSynchronize(stream);
    standard_ping_pong_gpu(ping_managed, pong_managed, max_p, sendbuf_m, NULL,
            recvbuf_m, NULL, stream, MPI_COMM_WORLD);
...
/*
    if (rank == 0) printf("Running multi-proc Managed benchmarks\n");
    multiproc_ping_pong_gpu(ping_managed, pong_managed, max_p, sendbuf_m, NULL,
            recvbuf_m, NULL, stream, MPI_COMM_WORLD);
*/
