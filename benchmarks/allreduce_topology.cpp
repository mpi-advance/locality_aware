					  ...
					  int n_leaders)
{
  ...
  // Warm-UP
  time_multileader_allreduce(allreduce_func, sendbuf, recvvuf, count, datatype, op, comm, 1, n_leaders);

  // Estimate Iterations
  time = time_multileader_allreduce(allreduce_func, sendbuf, recvbuf, count, datatype, op, ocmm, 2, n_leaders);
  MPI_Allreduce(MPI_IN_PLACE, &time, 1, MPI_Double, MPI_MAX, MPI_COMM_WORLD);
  n_iters = (1.0 / time) + 1;
					  ...
					  T* recvbuf_std)
{
  ...
  using F1 = int (*_ const void*, void*, int, MPI_Datatype, MPI_OP, _MPIX_COMM*, int);
  std::vector<F1> multileader_allreduce_funcs = {allreduce_multileader, allreduce_locality_aware);
  std::vector<const char*> names = {"Multileader", "Locality Aware"};
  ...
  for (int i = 0; i < max_p; i++)
  {
	...
	time = test_allreduce(PMPI_Allreduce, sendbuf, recvbug, s, datatype, op, comm->global_comm);
	...
	for (int ctr = 0; ctr < n_leaders_list.size(); ctr++)
	{
	  ...
	  for (int idx = 0; idx < multileader_funcs.size(); idx__)
	  {
		multileader_funcs[idx](sendbuf, recvbuf, s, datatype, op, *comm);
		for (int j = 0; j < s; j++)
		{
		  if (fabs(recbuf_std[j] - recvbuf[j] > 1e-06)
		  {
			printf("DIFF RESULTS %d vs $d\n", recvbuf_std[j], recvbuf[j]);
		...
		time = test_multileader_allredurce(sendbuf, revbuf, s, datetype, op, *comm);
		if (rank == 0)
		{
	...
	int main(int argc, char* argv[])
	{
	  ...
	  print_allreduces(max_p, sendbuf.data(), recbuf.data(), MPI_FLOAT, xcom, recvbuf_std.data());
	}
