...
def Create_One_Node_Test(m_name : str, algo : str, out_name : str):
  ...
  for i in range(2,9): 
    fp.write(f"srun --partition=normal --nodes=1 --ntasks={i} --time=24:00:00 ../../../build/benchmarks/torsten_standard_comm ../../../test_data/{m_name}.pm 1 {m_name} {algo}\n")
  for j in range(2,5):
    for i in range((j-1)*8+1, j*8+1):
      fp.write(f"srun --partition=normal --nodes={j} --ntasks={i} --time=24:00:00 ../../../build/benchmarks/torsten_standard_comm ../../../test_data/{m_name}.pm 1 {m_name} {algo}\n")
  fp.close()
