...
def Create_One_Node_Test(m_name : str, algo : str, out_name : str, num_tests : int):
  ...
  for k in range(2,33):
    fp.write(f"srun --partition=general --nodes=1 --ntasks={k} --time=00:30:00 ../../../build_hopper/benchmarks/torsten_standard_comm ../../../test_data/{m_name}.pm {num_tests} {m_name} {algo}\n")
  fp.close()
...
def Create_Many_Node_Test(m_name : str, algo : str, out_name : str, num_tests : int):
  ...
  for k in range(33,65):
    fp.write(f"srun --partition=general --nodes=2 --ntasks={k} --time=00:30:00 ../../../build_hopper/benchmarks/torsten_standard_comm ../../../test_data/{m_name}.pm {num_tests} {m_name} {algo}\n")
  fp.close()
