...
int main(int argc, char* argv[])
{
    ...
    int max_i = 20; // adjust to get larger sizes from here
    ...
    std::vector<double> pmpi_alltoallv(max_s*num_procs);
    std::vector<double> mpix_alltoallv(max_s*num_procs);
    ...
    for (int i = 0; i < max_i; i++)
    {
        ...
        tfinal = (MPI_Wtime() - t0) / n_iter;
        MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        if (rank == 0) printf("GPU-Aware Waitany Time %e\n", t0);
        ...
        tfinal = (MPI_Wtime() - t0) / n_iter;
        MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        if (rank == 0) printf("copy_to_cpu_alltoallv_pairwise_extra Time %e\n", t0);
        ...
        tfinal = (MPI_Wtime() - t0) / n_iter;
        MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        if (rank == 0) printf("copy_to_cpu_alltoallv_nonblocking_extra Time %e\n", t0);
