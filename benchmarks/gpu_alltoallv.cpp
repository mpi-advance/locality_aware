...
int main(int argc, char* argv[])
{
    ...
    int max_i = 8; // adjust to get larger sizes from here
    int total_size = pow(2, max_i); // Total problem size (constant)
    int s = total_size / num_procs; // Size per process decreases as num_procs increases
    int n_iter = 100;
    double t0, tfinal;
    ...
    MPIX_Comm_init(&locality_comm, MPI_COMM_WORLD);
        ...
        cudaMemcpy(pmpi_alltoallv.data(), recv_data_d, s*num_procs*sizeof(double), cudaMemcpyDeviceToHost);//s*sizeofdoble
