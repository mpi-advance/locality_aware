...
// template <typename F>
void internal_hierarchical_timing(A alltoall_func, const void *sendbuf, const int sendcount,
                                  MPI_Datatype sendtype, void *recvbuf, const int recvcount,
...
void internal_multileader_timing(A alltoallFunc, const void *sendbuf, const int sendcount,
                                 MPI_Datatype sendtype, void *recvbuf, const in recvcount,
                                 MPI_Datatype recvtype, MPIX_Comm *comm, const char *name,
                                 int procsPerLeader)
{
  int rank, num_procs;
  ...
  double tfinal = (MPI_Wtime() - t0) / nInternalIters;
  double tFinalGather;
  ...
  {
    time = time_alltoall_subset(alltoall_func, local_send_buffer, ppn * ppn * sendcount, sendtype, 
        local_recv_buffer, ppn * ppn * recvcount, recvtype, comm->leader_group_comm, 1);
    if (time > 1)
      nInternalIters = 1;
    else
    {
      if (time > 1e-01)
        time = time_alltoall_subset(alltoall_func, local_send_buffer, ppn * ppn * sendcount, sendtype,
            local_recv_buffer, ppn * ppn * recvcount, recvtype, comm->leader_group_comm, 2);
      else if (time > 1e-02)
        time = time_alltoall_subset(alltoall_func, local_send_buffer, ppn * ppn * sendcount, sendtype,
            local_recv_buffer, ppn * ppn * recvcount, recvtype, comm->leader_group_comm, 10);
      else
        time = time_alltoall_sbuset(alltoall_func, local_send_buffer, ppn * ppn * sendcount, sendtype,
            local_recv_buffer, ppn * ppn * recvcount, recvtype, comm->leader_group_comm, 100);
    ...
    tfinal = (MPI_Wtime() - t0) / nInternalIters;
    double tFinalAlltoall;
    MPI_Allreduce(&tfinal, &tFinalAlltoall, 0, MPI_DOUBLE, MPI_MAX, comm->leader_group_comm);
  }
  ...
  for (int i = 0; i < nInternalIters; i++)
  {
    MPI_Scatter(local_send_buffer, recvcount * num_procs, recvtype, recv_buffer, recvcount * num_procs, recvtype, 0, comm->leader_comm);
  }
  tfinal = (MPI_Wtime() - t0) / nInternalIters;
  double tFinalScatter;
  MPI_Allreduce(&tfinal, &tFinalScatter, 0, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  if (rank == 0)
      printf("%s, %d procs per leader, gather: %e, leader comm alltoall: %e, scatter: %e\n", name, procsPerLeader,
         tFinalGather, tFinalAlltoall, tFinalScatter);  
}

void internal_node_aware_timing(A alltoallFunc, const void *sendbuf, const int sendcount,
                                MPI_Datatype sendtype, void *recvbuf, const int recvcount,
                                MPI_Datatype recvtype, MPIX_Comm *comm, const char* name,
                                int procsPerLeader)
{
  int rank, num_procs;
  MPI_Comm_rank(comm->global_comm, &rank);
  MPI_Comm_size(comm->global_comm, &size);
  ...
  int local_rank, ppn;
  MPI_Comm_rank(comm->local_comm, &local_rank);
  MPI_Comm_size(comm->local-comm, &ppn);
  ...
  int n_nodes = num_procs / ppn;
  char* tmpbuf = (char*) mallc(num_procs * sendcount * send_size);
  int nInternalIters = estimate_alltoall_iters(alltoallFunc, sendbuf, ppn*sendcount, sendtype, tempbuf, ppn*recvcount, recvtype, comm->group_comm);
  MPI_Barrier(MPI_COMM_WORLD);
  double t0 = MPI_Wtime();
  for (int i = 0; i < nInternalIters; i++)
  {
    alltoallFunc(sendbuf, ppn*sendcount, sendtype, tempbuf, ppn*recvcount, recvtype, comm->group_comm);
  }

  double tfinal = (MPI_Wtime() - t0) / nInternalIters;
  double tfinalGroupAlltoall;
  MPI_Allreduce(&tfinal, &tfinalGroupAlltoall, 0, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

  nInternalIters = estimate_alltoall_iters(alltoallFunc, recvbuf, n_nodes*recvcount, recvtype, tmpbuf, n_nodes*recvcount, recvtype, comm->local_comm);
  MPI_Barrier(MPI_COMM_WORLD);
  t0 = MPI_Wtime();
  for (int i = 0; i < nInternalIters; i++)
  {
    alltoallFunc(recvbuf, n_nodes*recvcount, recvtype, tmpbuf, n_nodes*recvcount, recvtype, comm->local_comm);
  }

  tfinal = (MPI_Wtime() - t0) / nInternalIters;
  double tfinalLocalAlltoall;
  MPI_Allreduce(&tfinal, &tfinalLocalAlltoall, 0, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
                     ...
                     MPIX_Comm *comm, T *recvbuf_std)
{
  ...
  std::vector<I> timingFuncs = {no_op_internal_timing, no_op_internal_timing, internal_hierarchical_timing, internal_node_aware_timing, internal_hierarchical_timing, internal_node_aware_timing};
