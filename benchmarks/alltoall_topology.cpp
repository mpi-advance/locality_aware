...
double time_collective(F func, int n_iters, Args&&... args)
{
...
template <typename F, typename... Args>
int estimate_collective_iters(F collective_func, Args&&... args)
{
  double time = time_collective(collective_func, 1, std::forward<Args>(args)...);
  ...
  else
  {
    if (time > 1e-01)
      ...
      n_iters = 100;
    time = time_collective(collective_func, n_iters, std::forward<Args>(args)...);
    MPI_Allreduce(MPI_IN_PLACE, &time, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
...
template <typename F, typename... Args>
double test_collective(F func, Args&&... args)
{
  double time;
  int n_iters;

  // Warm-Up
  time_collective(func, 1, std::forward<Args>(args)...);

  // Estimate Iterations
  n_iters = estimate_collective_iters(func, std::forward<Args>(args)...);

  // Time Alltoall
  time = time_collective(func, n_iters, std::forward<Args>(args)...);
  MPI_Allreduce(MPI_IN_PLACE, &time, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  return time;
}
                     ...
                     MPIX_Comm *comm, T *recvbuf_std)
{
  ...
  for (int i = 0; i < max_p; i++)
  {
    ...
    time = test_collective(PMPI_Alltoall, sendbuf, s, sendtype,
    ...
    **************************************/
    ...
    time = test_collective(alltoall_hierarchical_pairwise, sendbuf, s, sendtype, recvbuf, s, recvtype, comm);
    if (rank == 0) printf("Hierarchical Pairwise: %e\n", time);

    // 2. Full hierarchical nonblocking alltoall
    time = test_collective(alltoall_hierarchical_nonblocking, sendbuf, s, sendtype, recvbuf, s, recvtype, comm);
    if (rank == 0) printf("Hierarchical Nonblocking: %e\n", time);

    // 3: Gather to rank 0 on local comm
    time = test_collective(MPI_Gather, sendbuf, s*num_procs, sendtype, local_sendbuf, s*num_procs, sendtype, 0, comm->local_comm);
    if (rank == 0) printf("Hierarchical Internal: Gather: %e\n", time);

    // 4. Scatter from rank 0 on local comm
    time = test_collective(MPI_Scatter, local_recvbuf, s*num_procs, sendtype, recvbuf, s*num_procs, sendtype, 0, comm->local_comm);
    if (rank == 0) printf("Hierarchical Internal: Scatter: %e\n", time);

    // 5. Pairwise alltoall between leaders on group comm
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(pairwise_helper, local_sendbuf, s*ppn*ppn, sendtype, local_recvbuf, s*ppn*ppn, recvtype, comm->group_comm, tag);
    ...
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(nonblocking_helper, local_sendbuf, s*ppn*ppn, sendtype, local_recvbuf, s*ppn*ppn, recvtype, comm->group_comm, tag);
    ...
    **************************************/
    ...
    time = test_collective(alltoall_node_aware_pairwise, sendbuf, s, sendtype, recvbuf, s, recvtype, comm);
    if (rank == 0) printf("Node-Aware Pairwise: %e\n", time);
  
    // 2. Full nonblocking node-aware alltoall
    time = test_collective(alltoall_node_aware_nonblocking, sendbuf, s, sendtype, recvbuf, s, recvtype, comm);
    if (rank == 0) printf("Node-Aware Nonblocking: %e\n", time);

    // 3. Pairwise intra-node alltoall
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(pairwise_helper, sendbuf, s*ppn, sendtype, recvbuf, s*ppn, recvtype, comm->local_comm, tag);
    if (rank == 0) printf("Node-Aware Internal: Pairwise intra-node alltoall: %e\n", time);

    // 4. Pairwise inter-node alltoall
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(pairwise_helper, sendbuf, s*n_nodes, sendtype, recvbuf, s*n_nodes, recvtype, comm->group_comm, tag);
    if (rank == 0) printf("Node-Aware Internal: Pairwise inter-node alltoall: %e\n", time);

    // 5. Nonblocking intra-node alltoall
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(nonblocking_helper, sendbuf, s*ppn, sendtype, recvbuf, s*ppn, recvtype, comm->local_comm, tag);
    if (rank == 0) printf("Node-Aware Internal: Nonblocking intra-node alltoall: %e\n", time);

    // 6. Nonblocking inter-node alltoall
    MPIX_Comm_tag(comm, &tag);
    time = test_collective(nonblocking_helper, sendbuf, s*n_nodes, sendtype, recvbuf, s*n_nodes, recvtype, comm->group_comm, tag);
    ...
    **************************************/
    ...
    for (int ctr = 0; ctr < procs_per_leader_list.size(); ctr++)
    {
      ...
      MPI_Comm_rank(comm->leader_comm, &my_leader);
      MPI_Comm_size(comm->leader_comm, &num_leaders);
      ...
      **************************************/
      ...
      time = test_collective(alltoall_multileader, pairwise_helper, sendbuf, s, sendtype, recvbuf, s, recvtype, comm, num_leaders);
      if (rank == 0) printf("Hierarchical Pairwise (N Leaders %d): %e\n", num_leaders, time);

      // 2. Full multileader nonblocking alltoall
      time = test_collective(alltoall_multileader, nonblocking_helper, sendbuf, s, sendtype, recvbuf, s, recvtype, comm, num_leaders);
      if (rank == 0) printf("Hierarchical Nonblocking (N Leaders %d): %e\n", num_leaders, time);

      // 3. Gather to rank 0 on leader comm
      time = test_collective(MPI_Gather, sendbuf, s*num_procs, sendtype, local_sendbuf, s*num_procs, sendtype, 0, comm->leader_comm);
      if (rank == 0) printf("Hierarchical (N Leaders %d) Internal: Gather: %e\n", num_leaders, time);

      // 4. Scatter from rank 0 on leader comm
      time = test_collective(MPI_Scatter, local_recvbuf, s*num_procs, sendtype, recvbuf, s*num_procs, sendtype, 0, comm->leader_comm);
      if (rank == 0) printf("Hierarchical (N Leaders %d) Internal: Scatter: %e\n", num_leaders, time);

      // 5. Pairwise alltoall between leaders on leader_group_comm
      time = test_collective(pairwise_helper, local_sendbuf, s*leader_size*leader_size, sendtype, local_recvbuf, s*leader_size*leader_size, recvtype, comm->leader_group_comm, tag);
      ...
      time = test_collective(nonblocking_helper, local_sendbuf, s*leader_size*leader_size, sendtype, local_recvbuf, s*leader_size*leader_size, recvtype, comm->leader_group_comm, tag);
      ...
      **************************************/
      ...
      time = test_collective(alltoall_locality_aware, pairwise_helper, sendbuf, s, sendtype, recvbuf, s, recvtype, comm, num_leaders);
      if (rank == 0) printf("Locality-Aware (N Groups %d) Alltoall Pairwise: %e\n", num_leaders, time);

      // 2. Full nonblocking locality-aware alltoall
      time = test_collective(alltoall_locality_aware, nonblocking_helper, sendbuf, s, sendtype, recvbuf, s, recvtype, comm, num_leaders);
      if (rank == 0) printf("Locality-Aware (N Groups %d) Alltoall Nonblocking: %e\n", num_leaders, time);

      // 3. Pairwise Intra-node alltoall
      MPIX_Comm_tag(comm, &tag);
      time = test_collective(pairwise_helper, sendbuf, s*leader_size, sendtype, recvbuf, s*leader_size, recvtype, comm->leader_comm, tag);
      if (rank == 0) printf("Locality-Aware (N Groups %d) Internal: Intra-node pairwise alltoall: %e\n", num_leaders, time);

      // 4. Pairwise Inter-node alltoall
      MPIX_Comm_tag(comm, &tag);
      time = test_collective(pairwise_helper, sendbuf, s*num_leaders, sendtype, recvbuf, s*num_leaders, recvtype, comm->leader_group_comm, tag);
      if (rank == 0) printf("Locality-Aware (N Groups %d) Internal: Inter-node pairwise alltoall: %e\n", num_leaders, time);

      // 3. Nonblocking Intra-node alltoall
      MPIX_Comm_tag(comm, &tag);
      time = test_collective(nonblocking_helper, sendbuf, s*leader_size, sendtype, recvbuf, s*leader_size, recvtype, comm->leader_comm, tag);
      if (rank == 0) printf("Locality-Aware (N Groups %d) Internal: Intra-node nonblocking alltoall: %e\n", num_leaders, time);

      // 4. Nonblocking Inter-node alltoall
      MPIX_Comm_tag(comm, &tag);
      time = test_collective(nonblocking_helper, sendbuf, s*num_leaders, sendtype, recvbuf, s*num_leaders, recvtype, comm->leader_group_comm, tag);
