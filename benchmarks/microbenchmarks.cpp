...
void ping(
    char* sendbuf, char* recvbuf, int size, int proc, MPI_Comm comm, MPI_Request* req)
{
...
double ping_pong(pingpong_ftn f_ping,
                 pingpong_ftn f_pong,
                 int rank0,
                 int rank1,
                 int size,
...
double estimate_iters(pingpong_ftn f_ping,
                      pingpong_ftn f_pong,
                      int rank0,
                      int rank1,
                      int size,
...
{
    double time =
        ping_pong(f_ping, f_pong, rank0, rank1, size, sendbuf, recvbuf, 5, comm, req);
...
double time_ping_pong(pingpong_ftn f_ping,
                      pingpong_ftn f_pong,
                      int rank0,
                      int rank1,
                      int size,
...
{
    int n_iters =
        estimate_iters(f_ping, f_pong, rank0, rank1, size, sendbuf, recvbuf, comm, req);
    double time = ping_pong(
        f_ping, f_pong, rank0, rank1, size, sendbuf, recvbuf, n_iters, comm, req);
    return time;
}

void print_ping_pong(pingpong_ftn f_ping,
                     pingpong_ftn f_pong,
                     int max_p,
                     int rank0,
                     int rank1,
                     ...
                     MPI_Request* req)
{
    ...
    for (int i = 0; i < max_p; i++)
    {
        ...
        double time =
            time_ping_pong(f_ping, f_pong, rank0, rank1, s, sendbuf, recvbuf, comm, req);
        MPI_Allreduce(MPI_IN_PLACE, &time, 1, MPI_DOUBLE, MPI_MAX, comm);
        if (rank == 0)
        {
            printf("%e\n", time);
...
void standard_ping_pong(int max_p, char* sendbuf, char* recvbuf)
{
    ...
    rank1 = PPNUMA / 2;
    print_ping_pong(
        ping, pong, max_p, rank0, rank1, sendbuf, recvbuf, MPI_COMM_WORLD, NULL);

    if (rank == 0)
    {
        printf("On-Socket Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPS / 2;
    print_ping_pong(
    ...
    if (rank == 0)
    {
        printf("On-Node, Off-Socket Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPN / 2;
    print_ping_pong(
...
void multiproc_ping_pong_step(
    int max_p, char* sendbuf, char* recvbuf, int max_n, int diff, bool cond0, bool cond1)
{
    int rank, rank0, rank1;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
void multiproc_ping_pong(int max_p, char* sendbuf, char* recvbuf)
{
    ...
    if (rank == 0)
    {
        printf("On-NUMA MultiProc Ping-Pong:\n");
    }
    size  = PPNUMA / 2;
    cond0 = rank < size;
    cond1 = rank < 2 * size;
    ...
    if (rank == 0)
    {
        printf("On-Socket MultiProc Ping-Pong:\n");
    }
    size  = PPS / 2;
    cond0 = rank < size;
    cond1 = rank < 2 * size;
    ...
    if (rank == 0)
    {
        printf("On-Node, Off-Socket MultiProc Ping-Pong:\n");
    }
    size  = PPN / 2;
    cond0 = rank < size;
    cond1 = rank < 2 * size;
    ...
    if (rank == 0)
    {
        printf("Off-Node MultiProc Ping-Pong:\n");
    }
    size  = PPN;
    cond0 = rank < size;
    cond1 = rank < 2 * size;
    ...
    if (rank == 0)
    {
        printf("On-NUMA MultiProc Ping-Pong, All NUMAs Active:\n");
    }
    size  = PPNUMA / 2;
    ...
    if (rank == 0)
    {
        printf("On-Socket MultiProc Ping-Pong, All Sockets Active:\n");
    }
    size  = PPS / 2;
    ...
    if (rank == 0)
    {
        printf("Off-Node MultiProc Ping-Pong, Even NUMA Regions:\n");
    }
    size  = PPNUMA;
    cond0 = rank < PPN;
    cond1 = rank < 2 * PPN;
    ...
    if (rank == 0)
    {
        printf("Off-Node MultiProc Ping-Pong, Even Sockets:\n");
    }
    size  = PPS;
    cond0 = rank < PPN;
    cond1 = rank < 2 * PPN;
...
void multi_ping_pong(int max_p, char* sendbuf, char* recvbuf, MPI_Request* req)
{
    ...
    if (rank == 0)
    {
        printf("On-NUMA Multi Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPNUMA / 2;
    print_ping_pong(multi_ping,
                    multi_pong,
                    max_p,
                    rank0,
                    rank1,
    ...
    if (rank == 0)
    {
        printf("On-Socket Multi Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPS / 2;
    print_ping_pong(multi_ping,
    ...
    if (rank == 0)
    {
        printf("On-Node, Off-Socket Multi Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPN / 2;
    print_ping_pong(multi_ping,
    ...
    if (rank == 0)
    {
        printf("Off-Node Multi Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPN;
    print_ping_pong(multi_ping,
...
void matching_ping_pong(int max_p, char* sendbuf, char* recvbuf, MPI_Request* req)
{
    ...
    if (rank == 0)
    {
        printf("On-NUMA Matching Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPNUMA / 2;
    print_ping_pong(matching_ping,
                    matching_pong,
                    max_p,
                    rank0,
                    rank1,
    ...
    if (rank == 0)
    {
        printf("On-Socket Matching Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPS / 2;
    print_ping_pong(matching_ping,
    ...
    if (rank == 0)
    {
        printf("On-Node, Off-Socket Matching Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPN / 2;
    print_ping_pong(matching_ping,
    ...
    if (rank == 0)
    {
        printf("Off-Node Matching Ping-Pong:\n");
    }
    rank0 = 0;
    rank1 = PPN;
    print_ping_pong(matching_ping,
